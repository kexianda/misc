...
if [ ${USER_ID:-0} -ne 0 ]; then
    if id -u ${USER_NAME} >/dev/null 2>&1; then userdel -f ${USER_NAME}; fi
    useradd --no-log-init -u ${USER_ID} ${USER_NAME} -s /bin/bash
    install -d -m 0755 -o ${USER_NAME} /home/${USER_NAME}
...
docker run -d -p ${SSH_PORT}:22 -p 4444:4444 \
    --name ${CONTAINER_NAME} -h ${CONTAINER_NAME} \
    --cap-add=SYS_PTRACE --security-opt seccomp=unconfined  \
    -v ~/../:/home/ -v ~/build/vscode/.vscode-server/:/home/${USER_NAME}/.vscode-server \
